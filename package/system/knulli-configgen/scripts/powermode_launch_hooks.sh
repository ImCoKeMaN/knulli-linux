...
handle_game_stop() {
    ...
    if is_power_connected; then
        POWER_MODE="$(/usr/bin/knulli-settings-get-master global.powermode)"
    else
        POWER_MODE="$(/usr/bin/knulli-settings-get-master global.batterymode)"
    fi
...
handle_game_start() {
    ...
    if [ -n "${GAME_NAME}" ]; then
        ...
        POWER_MODE="$(/usr/bin/knulli-settings-get-master "${POWER_MODE_SETTING}")"
    fi
