...
# the knulli-amd-tdp scripts to allow adjustments to TDP
...
CPU_TDP=$(/usr/bin/knulli-settings-get system.cpu.tdp)
...
handle_tdp() {
    TDP_PERCENTAGE=$1
    ROM_NAME=$2
    MAX_TDP=$(/usr/bin/knulli-settings-get system.cpu.tdp)
...
if [ "$EVENT" == "gameStop" ]; then
    ...
    TDP_SETTING=$(printf "%.0f" "$(/usr/bin/knulli-settings-get global.tdp)")
    if [ -z "${TDP_SETTING}" ]; then
        TDP_SETTING="$(/usr/bin/knulli-settings-get system.cpu.tdp)"
...
if [ -n "${SYSTEM_NAME}" ]; then
    ...
    TDP_SETTING=$(/usr/bin/knulli-settings-get "${SYSTEM_NAME}[\"${ROM_NAME}\"].tdp")
    if [ -z "${TDP_SETTING}" ]; then
        TDP_SETTING="$(/usr/bin/knulli-settings-get ${SYSTEM_NAME}.tdp)"
    fi
fi
...
if [ -z "${TDP_SETTING}" ]; then
    TDP_SETTING="$(/usr/bin/knulli-settings-get-master system.cpu.tdp)"
    if [ -n "${TDP_SETTING}" ]; then
