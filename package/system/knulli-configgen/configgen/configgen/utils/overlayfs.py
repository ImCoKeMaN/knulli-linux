...
def _unmount_and_remove(mount_point: Path):
    if mount_point.is_mount():
        ...
        if result.returncode != 0:
            eslog.error("failed unmounting '%s' (rc=%d) because %s",
...
def _mount(read_only_dir: Path, writable_upper_dir: Path, writable_work_dir: Path, mount_point: Path) -> bool:
    ...
    if result.returncode == 0:
        eslog.debug("mounted '%s' with components '%s'",
                       mount_point, components)
    else:
        eslog.error("failed mounting '%s' with components '%s' (rc=%d) because %s",
...
def mount_overlayfs(read_only_dir: Path, writable_dir: Path, /) -> Generator[Path]:
    ...
    The main use-case here will be supporting writes on top of a
    ...
    if read_only_dir.is_file():
        eslog.debug("overlaying single-file or linked rom '%s'", read_only_dir)
