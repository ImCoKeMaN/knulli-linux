...
define SLANG_SHADERS_INSTALL_TARGET_CMDS
	mkdir -p $(TARGET_DIR)/usr/share/knulli/shaders
	$(MAKE) CXX="$(TARGET_CXX)" -C $(@D) \
	    INSTALLDIR=$(TARGET_DIR)/usr/share/knulli/shaders install
endef

define SLANG_SHADERS_BATOCERA_SHADERS_SLANG
    # Some shaders got the .slan(g) variants moved
    mkdir -p $(TARGET_DIR)/usr/share/knulli/shaders/interpolation/shaders
    mkdir -p $(TARGET_DIR)/usr/share/knulli/shaders/scalehq/shaders
    cd $(TARGET_DIR)/usr/share/knulli/shaders/ && \
