...
# run manually:  board/scripts/doWinepackage.sh "output/target" "output/wine" "output/images/batocera"
...
XTARGET_VERSION=$(cat "${G_TARGETDIR}/usr/share/batocera/knulli.version" | sed -e s+" .*$"++)
XTARGET_ARCH=$(cat "${G_TARGETDIR}/usr/share/batocera/knulli.arch")
