...
initialize_settings() {
    ...
    if [[ -z "$MODE" || ! "$MODE" =~ ^(dim|dispoff|suspend|shutdown|none)$ ]]; then
        ...
        /usr/bin/knulli-settings-set system.batterysaver.mode "$MODE"
    fi

    TIMER="$(/usr/bin/knulli-settings-get system.batterysaver.timer)"
    if [[ -z "$TIMER" || ! "$TIMER" =~ ^[0-9]+$ || "$TIMER" -lt 60 ]]; then
        TIMER="300" # default in seconds
        /usr/bin/knulli-settings-set system.batterysaver.timer "$TIMER"
    fi
    ...
    if [[ -z "$EXTENDED_MODE" || ! "$EXTENDED_MODE" =~ ^(suspend|shutdown|none)$ ]]; then
        ...
        /usr/bin/knulli-settings-set system.batterysaver.extendedmode "$EXTENDED_MODE"
    fi

    EXTENDED_TIMER="$(/usr/bin/knulli-settings-get system.batterysaver.extendedtimer)"
    if [[ -z "$EXTENDED_TIMER" || ! "$EXTENDED_TIMER" =~ ^[0-9]+$ || "$EXTENDED_TIMER" -lt 60 ]]; then
        ...
        /usr/bin/knulli-settings-set system.batterysaver.extendedtimer "$EXTENDED_TIMER"
    fi
    ...
    if [[ -z "$AGGRESSIVE" || ! "$AGGRESSIVE" =~ ^(1|0)$ ]]; then
        ...
        /usr/bin/knulli-settings-set system.batterysaver.aggressive "$AGGRESSIVE"
    fi
}
    ...
    case "$MODE" in
        dim)
            ...
            if [ "$AGGRESSIVE" == "1" ]; then
                ...
                if [ "$(knulli-settings-get global.retroachievements)" = "0" ]; then
